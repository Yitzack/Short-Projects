...
{
	memcpy(SBox, other.SBox, 256);
	memcpy(InvSBox, other.InvSBox, 256);
	memcpy(Expanded_Key, other.Expanded_Key, 240);
}

// Move constructor
AES::AES(AES&& other) noexcept
{
	move(begin(other.SBox), end(other.SBox), begin(SBox));
	move(begin(other.InvSBox), end(other.InvSBox), begin(InvSBox));
	move(begin(other.Expanded_Key), end(other.Expanded_Key), begin(Expanded_Key));
	//reconstruct arrays
}
...
AES& AES::operator=(const AES& other)
{
	if (this != &other)
	{
		memcpy(this->SBox, other.SBox, 256);
		memcpy(this->InvSBox, other.InvSBox, 256);
		memcpy(this->Expanded_Key, other.Expanded_Key, 240);
	}
	return(*this);
}
...
void AES::KeyAddition(GF256 Text[], const GF256* Expanded_Key, int start_Text) const
{
	for(int i = 0; i < 16; i++)
	{
